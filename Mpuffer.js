...
const { ensureTable, isSeen, markAsSeen } = require("./db_Mpuffer");

const TELEGRAM_TOKEN = process.env.TELEGRAM_TOKEN;
const CHAT_ID = process.env.TELEGRAM_CHAT_ID;

const POSHMARK_URL =
  "https://poshmark.com/search?query=the%20north%20face%20jacket&sort_by=added_desc&department=Men&category=Jackets_%26_Coats&sub_category=Puffers&brand%5B%5D=The%20North%20Face&size%5B%5D=L&size%5B%5D=M&price%5B%5D=-65";
      ...
      if (item.title && item.price && item.size) {
        ...
        if (
          ...
          if (firstMatch) {
            ...
            await sendTelegramMessage(
              "🔔 *You got new deals!*\n\nHere are the latest Men Puffer JACKETS:"
            );
            firstMatch = false;
          }
